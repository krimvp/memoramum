...
- **API core** is the single enforcement point: both facades route through it; nothing reaches Postgres except through it (plus RLS as defense-in-depth). REST callers authenticate with principal-bound bearer tokens ([ADR-0014](adr/0014-bearer-token-rest-auth.md)); MCP sessions get their principal pair from the launching surface integration ([ADR-0005](adr/0005-standalone-service-mcp.md)) or — connecting remotely over streamable HTTP — from the credential at the door, with the flow asserted per request in headers or once in the connect URL ([ADR-0015](adr/0015-remote-mcp-streamable-http.md), [ADR-0017](adr/0017-connect-url-flow-context.md)). Remote clients log in with OAuth 2.1 against the deployment's authorization server, the endpoint acting as an OAuth resource server that publishes its protected-resource metadata and verifies access tokens locally against the server's keys ([ADR-0016](adr/0016-oauth-resource-server.md)); the same static bearer tokens as REST remain available for machine callers and IdP-less deployments.
...
| Caller identity | OAuth 2.1 resource server in front of an external IdP, principal-bound static tokens beside it | Remote clients log in without pre-shared secrets and prove delegation; machine callers and IdP-less deployments keep tokens ([ADR-0014](adr/0014-bearer-token-rest-auth.md), [ADR-0016](adr/0016-oauth-resource-server.md)). No user store of our own. |
...
| Revoking a credential has to take effect inside its lifetime, not at expiry | token introspection (RFC 7662) behind the same verification seam, or shorter token lifetimes first — the coupling was the reason local JWT verification won ([ADR-0016](adr/0016-oauth-resource-server.md)) |
