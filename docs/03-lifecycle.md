...
**The judge.** Whether a candidate and a neighbor state the same fact, conflicting facts, or different facts is a three-way question — `duplicate` / `contradiction` / `unrelated` — behind one pluggable seam. The deterministic dev judges (`exact`: duplicates only; `overlap`: token overlap, never deployed) leave real contradiction detection off. The **System One judge** ([ADR-0019](adr/0019-system-one-contradiction-judge.md)) asks that question as a single *choice* question over the pair and gets calibrated probabilities back, not a written verdict; a `contradiction` or `duplicate` counts only above a fixed confidence gate (0.8 — a false contradiction supersedes a good memory, a missed one only delays it), and any fault (no answer, a malformed one, a timeout) is `unrelated`, so the write continues as it would without a model and the consolidator, the `wrong` signal and the review queue remain the paths in. The probabilities, the confidence, the model id and the token usage of a counted verdict ride on the `PROPOSE` event as `details.judge` — the evidence a reviewer sees next to `supersedes` / `contradicts`.
