...
| **Harness** | The runtime that hosts an agent and its MCP client (a surface integration, an IDE, an agent framework). It launches or connects to the MCP facade, holds the agent's credential — a principal-bound bearer token, or an OAuth access token it obtains by logging the user in — and asserts the flow — agents themselves never hold credentials. |
