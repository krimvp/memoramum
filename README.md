...
| **System One judge** | The model contradiction judge (`MEMORAMUM_JUDGE=jev`): one `choice` question — `duplicate` / `contradiction` / `unrelated` — answered with calibrated probabilities, counted only above a fixed confidence gate, `unrelated` on any fault ([doc 03 §3](docs/03-lifecycle.md), [ADR-0019](docs/adr/0019-system-one-contradiction-judge.md)). |
...
| **Retrieval leg** | One of the three candidate sources retrieval fuses: *lexical* (stemmed text search), *vector* (embedding similarity), *literal* (trigram word similarity over raw content, for paths and identifiers). Each leg admits only candidates clearing its own absolute relevance gate. |
| **Score weight** | An exponent policy sets on one of the five retrieval factors (relevance, retention, trust, status, scope proximity): `0` disables it, `1` is the default, `> 1` sharpens it. Weights reorder results; they never widen what a read may return — that is the read gates' job. |
